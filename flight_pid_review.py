...
#         tk.Label(root, textvariable=self.status, bd=1, relief=tk.SUNKEN, anchor=tk.W).pack(fill=tk.X)
...
#     def analyze_data(self, csv_file):
#         try:
#             self.status.set(f"Processing: {os.path.basename(csv_file)}...")
#             self.root.update()
...
#             self.canvas.draw()
#             self.update_metrics(metrics)
#             self.status.set(f"Done: {os.path.basename(csv_file)}")
