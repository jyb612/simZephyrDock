	...
	: Node("aruco_tracker_node")
{
    ...
    _aruco_id_sub = this->create_subscription<std_msgs::msg::Int32>(
        "/aruco_id", bool_qos, std::bind(&ArucoTrackerNode::aruco_id_callback, this, std::placeholders::_1));

    _marker_size_sub = this->create_subscription<std_msgs::msg::Float32>(
        "/marker_size", bool_qos, std::bind(&ArucoTrackerNode::marker_size_callback, this, std::placeholders::_1));
	
	// Publishers
	_image_pub = create_publisher<sensor_msgs::msg::Image>(image_proc_topic, bool_qos);
	_target_pose_pub = create_publisher<geometry_msgs::msg::PoseStamped>(target_pose_topic, lidar_qos);
	_isloaded_pub = create_publisher<std_msgs::msg::Bool>("/isloaded", bool_qos);
	_aruco_detected_pub = create_publisher<std_msgs::msg::Bool>(aruco_detected_topic, bool_qos);
}
...
void ArucoTrackerNode::image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
{
	try {
		...
		RCLCPP_INFO(get_logger(), "%s", _camera_namespace.c_str());
